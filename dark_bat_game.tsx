...
const DarkBatGame = () => {
  ...
  const skins = {
    default: { name: 'Morcego Clássico', price: 0, color: '#1a0a0a', eyeColor: '#ff0000', rarity: 'comum' },
    ...
    demon: { name: 'Demônio', price: 150, color: '#8b0000', eyeColor: '#ff6600', rarity: 'incomum' },
    ...
    skeleton: { name: 'Esqueleto Alado', price: 1000, color: '#e8e8e8', eyeColor: '#00ff00', rarity: 'lendário', special: true }
  };
        ...
        <div className="bg-gradient-to-b from-gray-900 to-black border-4 border-red-900 rounded-lg p-8 max-w-2xl w-full" style={{
          ...
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(skins).map(([skinId, skinData]) => {
              ...
              const isLegendary = skinData.rarity === 'lendário';
        ...
        >
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-10">
            <div className="bg-black bg-opacity-70 rounded-lg px-3 py-2 border-2 border-red-800">
              <div className="text-yellow-400 font-bold text-xl flex items-center gap-2">
                💰 {gameStarted ? collectedCoins : totalCoins}
              </div>
            </div>
          ...
          {(!gameStarted || gameOver) && (
            <div className={`absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-90 ${glitchEffect ? 'animate-pulse' : ''}`}>
              ...
              {gameOver && (
                <>
                  <p className="text-3xl text-red-500 mb-2">VOCÊ MORREU</p>
                  <p className="text-2xl text-red-400 mb-2">Pontos: {score}</p>
                  <p className="text-xl text-yellow-400 mb-6">💰 +{collectedCoins} moedas</p>
                </>
              )}
              ...
              <p className="text-red-400 text-sm">Clique ou pressione ESPAÇO para voar</p>
              <div className="mt-4 bg-black bg-opacity-50 rounded px-4 py-2 border border-yellow-600">
                <p className="text-yellow-400 text-sm">💰 Total de moedas: {totalCoins}</p>
              </div>
            </div>
          )}
        </div>
